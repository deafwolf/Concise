...
template <bool wahmode> void serializetest() {
  typedef ConciseSet<wahmode> Concise;
  std::cout << "[[[" << __PRETTY_FUNCTION__ << "]]]" << std::endl;

  {
    Concise source;
    size_t size = source.getSizeInBytes();
    char *buf = (char *)malloc(size);

    size_t write_size = source.write(buf);
    assert(write_size == size);

    Concise target;
    bool read_ok = Concise::read(buf, size, target);
    assert(read_ok);
    assert(source.last == target.last);
    assert(source.lastWordIndex == target.lastWordIndex);
    assert(target.words.size() == source.words.size());
    assert(target.equals(source));

    bool fail_1 = Concise::read(buf, size - 1, target);
    assert(fail_1 == false);
    bool fail_2 = Concise::read(buf, size + 1, target);
    assert(fail_2 == false);
